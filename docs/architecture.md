...
`src/visualizations/observability-stream.ts` owns the observability page’s snapshot, WebSocket recovery, latest validated projection, disconnected expiry, and browser-cadence notification. `src/visualizations/observability.ts` owns persistent aggregate, time-bucket, duration-band, selector, and SVG waterfall nodes. Bucket selection is presentation state; the server remains the owner of aggregation and sample choice. The selected trace waterfall anchors its display clock to the root request and derives one correction per service at the first cross-service parent-child boundary. This contains cross-runtime clock skew, preserves elapsed time within each service, and leaves stored timestamps unchanged. The view then focuses on the initial contiguous activity: because span duration already covers network latency, a gap longer than one second ends that chart scale. Spans after the gap remain joined and appear as keyed later activity with their request-relative offsets, so tab suspension, interaction, and reconnect activity cannot flatten the request machinery. Reducer application and final paint are local-only spans, so applying the projection cannot feed the stream it displays.
...
`src/visualizations/tail-latency.ts` owns article-specific rendering. It updates persistent area and line paths plus one amber latest-point circle; historical requests remain in path geometry without separate DOM nodes. While a live chart is visible, one animation-frame loop recomputes bounded geometry against the current presentation clock, so points move continuously between projections. It owns at most one pending frame, stops when no point remains, cancels work off-screen, and falls back to expiry timers when `prefers-reduced-motion` is active. It never replaces the SVG or renders a point outside the declared window. The server-rendered version 2 projection remains as the static fallback.
